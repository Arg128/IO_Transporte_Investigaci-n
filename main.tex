...
{\large\bfseries ESCUELA PROFESIONAL DE INGENIER\'IA DE SISTEMAS}\\[1.5cm]

\begin{tikzpicture}
\draw[unsared, line width=1.2pt] (0,0) circle (1.7cm);
\node[unsared, font=\Huge\bfseries] at (0,0) {IO};
\end{tikzpicture}\\[1cm]

{\Large\bfseries Investigaci\'on de Operaciones}\\[1.8cm]
...
% ================================================================
\section{Algoritmo para la soluci\'on del modelo}
% ================================================================
...
\subsection{Implementaci\'on en el software}
...
% ================================================================
La soluci\'on \'optima concentra el env\'io de Majes casi \'integramente hacia Matarani (900 de sus 1\,100 toneladas), la ruta de menor costo y menor riesgo del sistema. Caman\'a destina toda su producci\'on a Lima, ruta larga pero la m\'as barata disponible para ese centro de acopio dado que su costo hacia Matarani, aunque m\'as corto, no puede absorber por s\'i solo la demanda de Lima. La Joya, por su ubicaci\'on relativa, atiende \'integramente Cusco y Puno, evitando el tramo m\'as costoso y riesgoso hacia Lima.

\begin{itemize}
  \item \textbf{¿Qu\'e decisiones deber\'ian tomarse?} Consolidar el eje Majes--Matarani como ruta principal, y reservar la flota con mejores condiciones de seguridad (frenos ABS, GPS, choferes con mayor antig\"uedad) para las rutas de Lima y Cusco, que concentran el mayor riesgo acumulado.
  \item \textbf{¿Qu\'e beneficios genera la soluci\'on?} Respecto a una asignaci\'on emp\'irica sin criterio de optimizaci\'on (que en la pr\'actica tiende a repartir carga de forma proporcional a la oferta de cada centro), el modelo reduce el costo total y, expl\'icitamente, evita rutas de alto riesgo cuando existen alternativas de costo similar.
  \item \textbf{¿Es una soluci\'on viable?} S\'i: respeta la capacidad de acopio, la demanda de cada destino y no requiere inversi\'on adicional, solo un cambio en el criterio de asignaci\'on de las \'ordenes de despacho.
  \item \textbf{¿Qu\'e limitaciones presenta?} El modelo asume costos y riesgos constantes durante el mes, no captura la disponibilidad real de camiones por d\'ia, ni las condiciones clim\'aticas variables del corredor Arequipa--Puno (relevantes en la temporada de heladas, seg\'un lo reportado por la Polic\'ia de Carreteras (ONSV, 2026)).
\end{itemize}
...
\subsection*{Escenario A: incremento de 20\% en el costo de combustible}
...
\subsection*{Escenario B: reducci\'on de 30\% en la oferta de Majes (estr\'es h\'idrico)}
...
% ================================================================
\section{Reflexi\'on sobre el proceso de modelaci\'on}
% ================================================================
...
\item \textbf{¿Qu\'e supuestos realizaron para construir el modelo?} Balance exacto entre oferta y demanda mensual, linealidad del costo de flete, constancia del \'indice de riesgo durante el periodo de an\'alisis (ver Secci\'on~4.3).
...
\begin{itemize}
  \item El modelo de transporte con penalizaci\'on por riesgo permite reducir la exposici\'on a corredores viales cr\'iticos sin necesidad de restricciones expl\'icitas de seguridad, incorporando dicho criterio directamente en el costo.
  \item La soluci\'on \'optima concentra los env\'ios de cada centro de acopio en uno o dos destinos principales, en lugar de repartir la carga proporcionalmente, lo que reduce el n\'umero de rutas activas y facilita el control operativo.
  \item El sistema es robusto ante variaciones en la disponibilidad de acopio, pero vulnerable a incrementos generalizados en el costo de combustible.
\end{itemize}

\subsection*{Aportes del modelo}
Provee a la empresa un criterio cuantitativo, replicable y auditable para la asignaci\'on mensual de carga, reemplazando la asignaci\'on emp\'irica actual y dejando trazabilidad expl\'icita del \emph{trade-off} entre costo y seguridad vial.

\subsection*{Recomendaciones para la organizaci\'on}
\begin{enumerate}
  \item Adoptar el plan \'optimo de env\'ios como pol\'itica de asignaci\'on mensual, revis\'andolo cada vez que cambien costos de flete o demanda de los destinos.
  \item Negociar con la aseguradora coberturas diferenciadas por ruta, dado que el modelo cuantifica expl\'icitamente qu\'e rutas concentran mayor riesgo.
  \item Explorar contratos de combustible a precio fijo, dado que el Escenario A mostr\'o que este es el par\'ametro que m\'as impacta el costo total.
  \item Priorizar el mantenimiento preventivo y la antig\"uedad de las unidades asignadas a las rutas Lima y Cusco, identificadas como las de mayor \'indice de riesgo.
\end{enumerate}

\subsection*{Posibles trabajos futuros}
Extender el modelo a un horizonte multiperiodo, incorporar variables binarias de selecci\'on de transportista y considerar un modelo de transporte con m\'ultiples productos (p\'aprika, palta, quinua) con requerimientos de conservaci\'on distintos.

\subsection*{``Lecciones aprendidas'' (reflexi\'on del equipo)}
\begin{itemize}
  \item \textbf{¿Qu\'e fue lo m\'as dif\'icil al modelar un problema real?} Traducir un criterio cualitativo (seguridad vial) en un par\'ametro num\'erico comparable con el costo monetario, sin caer en una simplificaci\'on excesiva.
  \item \textbf{¿Qu\'e habilidad consideran que desarrollaron m\'as?} La formulaci\'on de modelos matem\'aticos a partir de informaci\'on dispersa y de fuentes p\'ublicas heterog\'eneas (reportes de accidentalidad, normativa de transporte, datos operativos).
  \item \textbf{¿Qu\'e recomendar\'ian a un equipo que desarrolle este proyecto el pr\'oximo semestre?} Recolectar desde el inicio datos primarios de la propia organizaci\'on (tarifas reales de flete, historial de siniestros de la flota) para no depender \'unicamente de fuentes agregadas p\'ublicas.
\end{itemize}
